							 ...
							 e_mt_gen_alg mt_alg, e_sharing sharing, double afp, double bfp, std::string inputfile, std::string outputfile)
{
	...
	if(role == SERVER) {
		s_xin = bc->PutSIMDINGate(nvals, xembeddings.data(), bitlen, SERVER);
	} else { //role == CLIENT
	...
	s_yin = bc->PutSharedSIMDINGate(nvals, share_embeddings.data(), bitlen);
	...
	share *s_x_times_y = bc->PutMULGate(s_xin, s_yin);
	...
	for (int i = 1; i < nvals; i++)
	{
		...
		s_x_dot_y = bc->PutADDGate(s_x_dot_y , bc->PutSubsetGate(s_x_times_y,posids,1,true));
	...
	share *s_x_times_x = bc->PutMULGate(s_xin, s_xin);
	...
	for (int i = 1; i < nvals; i++)
	{
		...
		// share *s_product_split;
		s_norm_x = bc->PutADDGate(s_norm_x , bc->PutSubsetGate(s_x_times_x,posids,1,true));
		//std::cout << "s_share nvals: " << a_share->get_nvals() << std::endl;
		//std::cout << "s_share bitlen: " << a_share->get_bitlength() << std::endl;
	...
	share *s_y_times_y = bc->PutMULGate(s_yin, s_yin);
	...
	for (int i = 1; i < nvals; i++)
	{
		...
		// share *s_product_split;
		s_norm_y = bc->PutADDGate(s_norm_y , bc->PutSubsetGate(s_y_times_y,posids,1,true));
		//std::cout << "s_share nvals: " << a_share->get_nvals() << std::endl;
		//std::cout << "s_share bitlen: " << a_share->get_bitlength() << std::endl;
		//bc->PutPrintValueGate(a_share, "a_share");
	}
