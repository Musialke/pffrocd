							 ...
							 e_mt_gen_alg mt_alg, e_sharing sharing, double afp, double bfp, std::string inputfile, std::string outputfile)
{
	...
	for (int i = 0; i < nvals; i++) {
		s_xin[i] = bc->PutINGate(xembeddings[i], bitlen, SERVER);
		s_yin[i] = bc->PutINGate(yembeddings[i], bitlen, CLIENT);
	}

	for (int i = 0; i < nvals; i++) {
		bc->PutPrintValueGate(s_xin[i], "s_xin[" + std::to_string(i) + "]");
	}

	for (int i = 0; i < nvals; i++) {
		bc->PutPrintValueGate(s_yin[i], "s_yin[" + std::to_string(i) + "]");
	}
