/**
 ...
 \brief		Implementation of the float add problem using ABY Framework.
 */
...
int32_t test_float_add_circuit(e_role role, const std::string& address, uint16_t port, seclvl seclvl,
	...
	*/
	...
	float alice_money, bob_money, output;
	srand(time(NULL));
	alice_money = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/100.0)); // random float between 0.0 and 100.0
	bob_money = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/100.0)); // random float between 0.0 and 100.0
	uint32_t *alice_money_ptr = (uint32_t*) &alice_money;
	uint32_t *bob_money_ptr = (uint32_t*) &bob_money;
	...
	*/
	...
	if(role == SERVER) {
		...
		s_bob_money = circ->PutINGate(bob_money_ptr, bitlen, SERVER);
	} else { //role == CLIENT
		s_alice_money = circ->PutINGate(alice_money_ptr, bitlen, CLIENT);
	...
	*/
	...
	s_out = BuildFloatAddCircuit(s_alice_money, s_bob_money,
			(BooleanCircuit*) circ);
	...
	*/
	...
	std::cout << "Testing float add in " << get_sharing_name(sharing)
				<< " sharing: " << std::endl;
	std::cout << "\nAlice Money:\t" << alice_money;
	std::cout << "\nBob Money:\t" << bob_money;
	std::cout << "\nCircuit Result:\t" << val;
	std::cout << "\nVerify Result: \t" << alice_money + bob_money
				<< "\n";
...
share* BuildFloatAddCircuit(share *s_alice, share *s_bob,
		BooleanCircuit *bc) {

	share* out;

	/** Calling the greater than equal function in the Boolean circuit class.*/
	out = bc->PutFPGate(s_alice, s_bob, ADD);
